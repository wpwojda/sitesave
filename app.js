...
// ── BOOT ─────────────────────────────────────────────────────
async function init() {
  // onAuthStateChange fires for both existing sessions AND OAuth callbacks.
  // We use it as the single source of truth and set a timeout fallback
  // in case no auth event fires (genuinely logged out).

  let resolved = false;

  // Set up the listener FIRST before any async calls
  sb.auth.onAuthStateChange(async (event, session) => {
    if (event === 'SIGNED_IN' && session?.user) {
      resolved = true;
      CURRENT_USER = session.user;
      // Clean the URL if it has an auth token in it
...
function enterGuest() {
  BM = [...PLACEHOLDERS];
  // Header: show sign-in button, hide save + avatar
...
async function enterApp() {
  if (_appEntered && CURRENT_USER) return;
  _appEntered = true;
  S.guestMode = false;
