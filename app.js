...
function showLogin() {
  ...
  document.getElementById('app-header-controls').classList.add('hidden');
}

function showApp() {
  document.getElementById('login-screen').classList.add('hidden');
  document.getElementById('app-shell').classList.remove('hidden');
  document.getElementById('app-header-controls').classList.remove('hidden');
  updateUserAvatar();
}
...
function updateUserAvatar() {
  ...
  if (pic) {
    avatar.innerHTML = `<img src="${pic}" alt="${name}" style="width:28px;height:28px;border-radius:50%;display:block;">`;
  } else {
...
async function loadBookmarks() {
  ...
  // Normalise rows — tags stored as jsonb array, date from created_at
  BM = (data || []).map(row => ({
    id:    row.id,
    url:   row.url,
    name:  row.name,
...
function deleteTagCategory(tag) {
  ...
  affected.forEach(b => {
    const newTags = b.tags.filter(t => t.toLowerCase() !== lower);
    b.tags = newTags;
    dbUpdate(b.id, { tags: newTags });
  });
...
async function saveBM() {
  ...
  if (S.editId) {
    ...
    if (row) {
      BM.unshift({
        id: row.id, url, name, tags,
        color: S.color, fav: false,
        date: new Date(row.created_at).getTime()
      });
    }
    toast('Saved');
